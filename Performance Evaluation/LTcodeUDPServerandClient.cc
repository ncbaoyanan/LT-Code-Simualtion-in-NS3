...
// the total length for a packet is 1008 bytes
...
//      std::cout<<"m_sendSize:"<<m_sendSize<<std::endl;
...
//for(; errrate < 0.01; errrate += 0.0003){
for(int i = 0; i < 30; i++){
	NodeContainer nodes;
	nodes.Create(2);
	...
	em->SetAttribute("ErrorRate", DoubleValue(errrate));// the error rate should be changed to see its effects
	...
	appS->SetStartTime(Seconds(1.));
	appS->SetStopTime(Seconds(1000.));
...
//	devices.Get(1)->TraceConnectWithoutContext("ReceivePkt", MakeCallback(&ReceivePkt));
	...
	std::cout<<"Finished"<<std::endl;
        ...
        std::cout<<"error rate:"<<errrate<<"    successd times:"<<succnt<<"    failed times"<<30-succnt<<std::endl;
        std::cout<<"total time:"<<total;
        std::cout<<"    average time:"<<total/succnt<<std::endl;
        total = 0;
        succnt = 0;
